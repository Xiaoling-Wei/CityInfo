...
// 删除过期付费信息
$id = $_GET['id'];
$sql2 = "delete from tb_leaguerinfo WHERE showday<DATE(NOW())";
...
if($state=="all"){
	...
	$sql1=mysqli_query($conn,"select count(*) as total from tb_leaguerinfo  where type='$type' and checkstate=$state order by id");
}
...
if($state=="all"){
	$sql=mysqli_query($conn,"select * from tb_leaguerinfo  where type='$type' order by id limit ".($page-1)*$pagesize.",$pagesize");
}else{
	$sql=mysqli_query($conn,"select * from tb_leaguerinfo  where type='$type' and checkstate=$state order by id limit ".($page-1)*$pagesize.",$pagesize");
}
