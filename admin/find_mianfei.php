...
if($state=="all"){
	...
	$sql1=mysqli_query($conn,"select count(*) as total from tb_info where type='$type' and checkstate=$state order by edate");
}
...
if($state=="all"){
	$sql=mysqli_query($conn,"select * from tb_info where type='$type' order by edate limit ".($page-1)*$pagesize.",$pagesize");
}else{
	$sql=mysqli_query($conn,"select * from tb_info where type='$type' and checkstate=$state order by edate limit ".($page-1)*$pagesize.",$pagesize");
}
