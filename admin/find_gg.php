...
if($flag=="all"){
	$sql=mysqli_query($conn,"select * from tb_advertising order by flag desc limit ".($page-1)*$pagesize.",$pagesize");
}else{
	$sql=mysqli_query($conn,"select * from tb_advertising where flag='$flag' order by fdate desc limit ".($page-1)*$pagesize.",$pagesize");
}
